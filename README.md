...
作为AI驱动的插件生成器，CodeMage具有强大的代码生成能力，但同时也带来了一定的安全风险：

**主要风险**：
- 生成的插件代码具有与其他插件相同的系统权限
- 错误或恶意的插件代码可能导致系统文件损坏或数据丢失
- 可能生成具有网络访问能力的插件，存在隐私泄露风险

**内置安全措施**：
- ✅ 管理员权限验证：仅允许管理员使用插件生成功能
- ✅ 反向提示词过滤：防止生成恶意或不当内容
- ✅ 代码审查机制：对生成的代码进行质量检查
- ✅ 用户确认流程：生成前需要用户明确确认
...
5. **使用规范**
   - 仅在完全私有、可信环境中运行本插件
   - 不要将插件暴露在公共网络或不受信任用户面前
   - 对生成的插件进行人工审查后再安装使用

> **注意**：插件生成的代码具有与运行用户相同的系统权限，请务必在安全可控的环境中使用此插件。
