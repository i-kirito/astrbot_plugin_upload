...
class DirectoryDetector:
    ...
    def detect_astrbot_installation(self) -> Optional[str]:
        """检测AstrBot安装目录
        ...
        if (current_dir.parent / "main.py").exists() and "astrbot_plugin_" in current_dir.name:
    ...
    def get_plugins_directory(self) -> Optional[str]:
        ...
        try:
            ...
            # 如果当前目录是插件目录（包含main.py），则从父目录开始搜索
            if (current_plugin_dir.parent / "main.py").exists() and "astrbot_plugin_" in current_plugin_dir.name:
            ...
            # 尝试获取AstrBot数据目录
            data_dir = os.path.join(os.path.dirname(os.path.dirname(str(current_plugin_dir))), "data")
    ...
    def validate_directory_structure(self) -> Dict[str, Any]:
        ...
        if not astrbot_root:
            ...
            if "astrbot_plugin_" in current_plugin_dir.name:
                ...
                if plugins_dir.name == "plugins" and \
                   data_dir.name == "data" and \
                   data_dir.parent.name != "":  # 确保不是根目录
